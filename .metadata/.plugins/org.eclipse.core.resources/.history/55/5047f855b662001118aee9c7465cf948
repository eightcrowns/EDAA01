...
public class CheckButton extends SudokuButton {
	...
	public void actionPerformed(ActionEvent e) {
		int[][] s = new int[9][9];
		// Läs in värden till ny vektor
		for (int y = 0; y < 9; y++) {
			for (int x = 0; x < 9; x++) {
				if (values[y][x] > 0) { // värdet var ifyllt från början
					s[y][x] = values[y][x]; // kopiera från ursprunglig vektor
				} else {
					// Läs in från textfält
					String field = fields[y][x].getText();
					if (field.isEmpty()) {
						s[y][x] = 0; // fältet var tomt - sätt till 0
					} else {
		...
		for (int y = 0; y < 9; y++) {
			for (int x = 0; x < 9; x++) {
				if (s[y][x] == 0 || !sudoku.check(y, x)) {
					...
					return; // avbryt så fort en felaktig ruta hittats
				}
			}
		}
